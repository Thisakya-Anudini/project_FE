    ...
    return(
        <div className="w-full h-[50px] flex flex-row justify-center items-center gap-[20px]">
            <select className="w-[100px] h-[40px] border border-gray-300 rounded-md p-[10px]" value={currentPage} onChange={(e)=>{
                setLoading(true);
                setCurrentPage(parseInt(e.target.value));//update the current page  if  something changed
            ...
            <select className="w-[100px] h-[40px] border border-gray-300 rounded-md p-[10px]" value={limit} onChange={(e)=>{  //update the limit
                setLoading(true);
